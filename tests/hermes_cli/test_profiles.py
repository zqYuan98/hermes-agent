...
class TestCreateProfile:
    ...
    def test_create_locks_root_source_and_destination(self, profile_env, monkeypatch):
        import contextlib

        root = profile_env / ".hermes"
        source = root / "profiles" / "source"
        destination = root / "profiles" / "clone"
        ...
        def fake_create_unlocked(name, **_kwargs):
            assert state["active"], "create implementation ran outside lifecycle locks"
            assert name == "clone"
            return destination

        monkeypatch.setattr(profiles, "profile_mutation_locks", tracked_locks)
        monkeypatch.setattr(profiles, "_create_profile_unlocked", fake_create_unlocked)

        result = create_profile("clone", clone_from="source", no_alias=True)

        assert result == destination
        assert lock_sets == [(root.resolve(), source.resolve(), destination.resolve())]
    ...
    def test_clone_all_copies_entire_tree(self, profile_env):
        ...
        (default_home / ".webui-profile-generation").write_text(
        ...
        # Runtime and source-incarnation files should be stripped.
        ...
        assert not (profile_dir / ".webui-profile-generation").exists()
...
class TestNoSkillsOptOut:
    ...
    def test_seed_profile_skills_is_reentrant_under_profile_lock(
        ...
        """Seeding must not wait on a child process for the lock held by its parent."""
        ...
        monkeypatch.setattr(
            ...
            lambda *args, **kwargs: (_ for _ in ()).throw(
                AssertionError("seeding must not spawn a lock-taking child process")
            ),
        )

        with profile_mutation_lock(profile_dir, timeout=1):
            result = seed_profile_skills(profile_dir, quiet=True)

        assert result is not None
        assert result["copied"] == ["demo"]
...
class TestBackfillProfileEnvs:
    ...
    def test_each_backfill_locks_root_and_target(self, profile_env, monkeypatch):
        import contextlib

        root = profile_env / ".hermes"
        target = root / "profiles" / "old1"
        ...
        def fake_backfill_unlocked(entry, default_env, quiet):
            assert state["active"], "env backfill ran outside lifecycle locks"
            assert Path(entry).resolve() == target.resolve()
            assert Path(default_env).resolve() == (root / ".env").resolve()
        ...
        monkeypatch.setattr(
            profiles,
            "_backfill_profile_env_unlocked",
            fake_backfill_unlocked,
            raising=False,
        )

        assert backfill_profile_envs(quiet=True) == ["old1"]
...
class TestDeleteProfile:
    ...
    def test_delete_locks_root_and_target(self, profile_env, monkeypatch):
        import contextlib

        root = profile_env / ".hermes"
        target = root / "profiles" / "coder"
        target.mkdir(parents=True)
        lock_sets = []
        state = {"active": False}
        ...
        def fake_delete_unlocked(name, yes=False):
            assert state["active"], "delete implementation ran outside lifecycle locks"
            assert (name, yes) == ("coder", True)
            return target

        monkeypatch.setattr(profiles, "profile_mutation_locks", tracked_locks)
        monkeypatch.setattr(profiles, "_delete_profile_unlocked", fake_delete_unlocked, raising=False)

        result = delete_profile("coder", yes=True)
...
class TestActiveProfile:
    ...
    def test_set_named_locks_root_and_target(self, profile_env, monkeypatch):
        import contextlib
        ...
        def fake_set_unlocked(name):
            assert state["active"], "active pointer RMW ran outside lifecycle locks"
            assert name == "coder"

        monkeypatch.setattr(profiles, "profile_mutation_locks", tracked_locks)
        monkeypatch.setattr(
            profiles, "_set_active_profile_unlocked", fake_set_unlocked, raising=False
        )
    ...
    def test_set_default_locks_root_only(self, profile_env, monkeypatch):
        import contextlib
        ...
        monkeypatch.setattr(profiles, "profile_mutation_locks", tracked_locks)
        monkeypatch.setattr(
            profiles, "_set_active_profile_unlocked", lambda name: None, raising=False
        )

        set_active_profile("default")

        assert lock_sets == [(root.resolve(),)]
...
class TestWrapperScript:
    ...
    def test_create_locks_root_and_target_profile(self, profile_env, monkeypatch):
        import contextlib

        root = profile_env / ".hermes"
        target = root / "profiles" / "redqueen"
        target.mkdir(parents=True)
        lock_sets = []
        state = {"active": False}
        ...
        expected = profile_env / ".local" / "bin" / "rq"

        def fake_create_unlocked(alias, profile):
            assert state["active"], "wrapper write ran outside lifecycle locks"
            assert (alias, profile) == ("rq", "redqueen")
            return expected

        monkeypatch.setattr(profiles, "profile_mutation_locks", tracked_locks)
        monkeypatch.setattr(
            profiles, "_create_wrapper_script_unlocked", fake_create_unlocked,
            raising=False,
        )

        result = create_wrapper_script("rq", target="redqueen")

        assert result == expected
        assert lock_sets == [(root.resolve(), target.resolve())]

    def test_remove_locks_root_wrapper_namespace(self, profile_env, monkeypatch):
        import contextlib
        ...
        @contextlib.contextmanager
        def tracked_lock(home):
            lock_sets.append(Path(home).resolve())
            state["active"] = True
            try:
                yield
            finally:
        ...
        def fake_remove_unlocked(alias):
            assert state["active"], "wrapper unlink ran outside root lock"
            assert alias == "rq"
            return True

        monkeypatch.setattr(profiles, "profile_mutation_lock", tracked_lock)
        monkeypatch.setattr(
            profiles, "_remove_wrapper_script_unlocked", fake_remove_unlocked,
            raising=False,
        )

        assert remove_wrapper_script("rq") is True
        assert lock_sets == [root.resolve()]
...
class TestRenameProfile:
    ...
    def test_rename_locks_root_old_and_new(self, profile_env, monkeypatch):
        import contextlib

        root = profile_env / ".hermes"
        old_dir = root / "profiles" / "oldname"
        new_dir = root / "profiles" / "newname"
        ...
        def fake_rename_unlocked(old_name, new_name):
            assert state["active"], "rename implementation ran outside lifecycle locks"
            assert (old_name, new_name) == ("oldname", "newname")
            return new_dir

        monkeypatch.setattr(profiles, "profile_mutation_locks", tracked_locks)
        monkeypatch.setattr(profiles, "_rename_profile_unlocked", fake_rename_unlocked, raising=False)

        result = rename_profile("oldname", "newname")

        assert result == new_dir
        assert lock_sets == [(root.resolve(), old_dir.resolve(), new_dir.resolve())]
    ...
    def test_rename_preserves_profile_generation(self, profile_env):
        old_dir = create_profile("oldname", no_alias=True)
        generation = "11111111-1111-4111-8111-111111111111\n"
        (old_dir / ".webui-profile-generation").write_text(generation)

        with patch("hermes_cli.profiles.check_alias_collision", return_value="skip"):
            new_dir = rename_profile("oldname", "newname")

        assert (new_dir / ".webui-profile-generation").read_text() == generation
...
class TestExportImport:
    ...
    def test_export_locks_source_profile(self, profile_env, tmp_path, monkeypatch):
        import contextlib
        ...
        def tracked_lock(profile_home):
            lock_keys.append(Path(profile_home).resolve())
            state["active"] = True
            try:
                yield
            finally:
        ...
        def fake_export_unlocked(name, output_path):
            assert state["active"], "export read ran outside the source Profile lock"
            assert (name, output_path) == ("coder", str(output))
            return output

        monkeypatch.setattr(profiles, "profile_mutation_lock", tracked_lock)
        monkeypatch.setattr(
            profiles, "_export_profile_unlocked", fake_export_unlocked, raising=False
        )

        result = export_profile("coder", str(output))

        assert result == output
        assert lock_keys == [profile_dir.resolve()]

    def test_import_stages_outside_then_locks_root_and_destination(
        ...
        with tarfile.open(archive, "w:gz") as tf:
            data = b"ok"
            info = tarfile.TarInfo("source/marker.txt")
            info.size = len(data)
            tf.addfile(info, io.BytesIO(data))

        root = profile_env / ".hermes"
        destination = root / "profiles" / "restored"
        lock_sets = []
        events = []
        state = {"active": False}
        ...
        def tracked_extract(archive_path, staging_root):
            assert not state["active"], "archive extraction held live Profile locks"
            events.append("extract")
            return real_extract(archive_path, staging_root)

        def fake_publish(canon, final_source, profile_dir):
            assert state["active"], "import publication ran outside lifecycle locks"
            assert canon == "restored"
            assert profile_dir == destination
            assert (final_source / "marker.txt").read_text() == "ok"
        ...
        result = import_profile(str(archive), name="restored")

        assert result == destination
        assert events == ["extract", "publish"]
        assert lock_sets == [(root.resolve(), destination.resolve())]
    ...
    def test_named_export_excludes_profile_generation(self, profile_env, tmp_path):
        profile_dir = create_profile("coder", no_alias=True)
        (profile_dir / ".webui-profile-generation").write_text(
            "11111111-1111-4111-8111-111111111111\n"
        )
        output = tmp_path / "coder.tar.gz"
        ...
        with tarfile.open(output, "r:gz") as tf:
            assert "coder/.webui-profile-generation" not in tf.getnames()
    ...
    def test_import_strips_legacy_profile_generation(self, profile_env, tmp_path):
        archive_path = tmp_path / "legacy.tar.gz"
        with tarfile.open(archive_path, "w:gz") as tf:
            generation = b"11111111-1111-4111-8111-111111111111\n"
            info = tarfile.TarInfo("legacy/.webui-profile-generation")
            info.size = len(generation)
            tf.addfile(info, io.BytesIO(generation))
            marker = b"ok"
            info = tarfile.TarInfo("legacy/marker.txt")
            info.size = len(marker)
            tf.addfile(info, io.BytesIO(marker))

        imported = import_profile(str(archive_path), name="restored")

        assert (imported / "marker.txt").read_text() == "ok"
        assert not (imported / ".webui-profile-generation").exists()
